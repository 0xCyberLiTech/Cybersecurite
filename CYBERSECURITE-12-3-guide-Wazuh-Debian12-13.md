...
- **OS :** Debian 12 (Bookworm) ou Debian 13 (Trixie), x86_64/ARM64.  
...
**Connexion :** ouvrez le navigateur sur `https://<IP_DU_SERVEUR>` (certificat auto‑signé par défaut), identifiant `admin`, mot de passe affiché ci‑dessus.
...
# (remplacer <ADMIN_PASSWORD> si vous l’avez modifié)
curl -k -u admin:<ADMIN_PASSWORD> https://localhost:55000/security/user/authenticate
```
...
sudo WAZUH_MANAGER="10.0.0.2" apt-get install -y wazuh-agent
...
- Sur l’endpoint, modifiez `/etc/hosts` ou créez un fichier dans un chemin surveillé (ex. `/etc/ssh/sshd_config`).  
...
for i in {1..6}; do ssh user@IP_AGENT 'exit'; done
```
